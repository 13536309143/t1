/*
 * File: src/rendering/cluster_lod_renderer.cpp
 * Purpose: Cluster LOD renderer implementation: traversal, software raster path, mesh shader path, and draw commands.
 */
...
bool RendererRasterClustersLod::initShaders(Resources& res, RenderScene& rscene, const RendererConfig& config)
{
  ...
  options.AddMacroDefinition("USE_FORCED_INVISIBLE_CULLING", "0"); // 强制隐藏面剔除 (此处写死为 0)
    ...
    res.compileShader(m_shaders.graphicsMesh, VK_SHADER_STAGE_MESH_BIT_NV, "passes/cluster/cluster_mesh.mesh.glsl", &options); // 编译 Mesh Shader
    res.compileShader(m_shaders.graphicsFragment, VK_SHADER_STAGE_FRAGMENT_BIT, "passes/cluster/cluster_shading.frag.glsl", &options); // 编译 Fragment Shader
    res.compileShader(m_shaders.computeTraversalPresort, VK_SHADER_STAGE_COMPUTE_BIT, "compute/traversal/traversal_presort.comp.glsl", &options); // 预排序 Compute
    res.compileShader(m_shaders.computeTraversalInit, VK_SHADER_STAGE_COMPUTE_BIT, "compute/traversal/traversal_init.comp.glsl", &options); // 初始化 Compute
    res.compileShader(m_shaders.computeTraversalRun, VK_SHADER_STAGE_COMPUTE_BIT, "compute/traversal/traversal_run.comp.glsl", &options); // LOD 遍历 Compute
    res.compileShader(m_shaders.computeBuildSetup, VK_SHADER_STAGE_COMPUTE_BIT, "compute/build/indirect_build_setup.comp.glsl", &options); // 间接绘制参数设置 Compute
    // 如果开启了软光栅，单独编译相应的软光栅 Compute Shader
  if(config.useComputeRaster)
  {
    res.compileShader(m_shaders.computeRaster, VK_SHADER_STAGE_COMPUTE_BIT, "passes/cluster/software_cluster_raster.comp.glsl", &options);
  }
  // 如果开启了分离遍历任务组，编译相关的 Compute Shader
  if(config.useSeparateGroups)
  {
    res.compileShader(m_shaders.computeTraversalGroups, VK_SHADER_STAGE_COMPUTE_BIT,"compute/traversal/traversal_run_separate_groups.comp.glsl", &options);
  }
  // 验证所有 Shader 是否都编译成功
...
void RendererRasterClustersLod::render(VkCommandBuffer cmd, Resources& res, RenderScene& rscene, const FrameConfig& frame, nvvk::ProfilerGpuTimer& profiler)
{
  ...
  for(uint32_t pass = 0; pass < passCount; pass++)
  {
    ...
    //对应代码：shaders/passes/fullscreen/hierarchical_z.comp.glsl。它通过 Subgroup 指令高效地对 Depth Texture 进行 Max/Min 规约，生成多个级别的 Mipmap。
